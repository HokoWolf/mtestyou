...
if(R::count('users') == 0)
{
	...
	$user1->login = 'admin';
	$user1->email = 'artem.tkach004@gmail.com';
	...
	$user3 = R::dispense('users');
	$user3->login = 'tester228';
	$user3->email = 'drugtester@gmail.com';
	$user3->password = password_hash('drugs', PASSWORD_DEFAULT);
	$user3->date = '2020-01-21 03:54:48';
	$user3->type = 'teacher';
	...
	$user4->login = 'you';
	$user4->email = 'you@gmail.com';
	$user4->password = password_hash('you', PASSWORD_DEFAULT);
	$user4->date = '2020-01-21 03:54:48';
	$user4->type = 'student';
	R::store($user4);
} # end of if(users)
...
if(R::count('topics') == 0)
{
	...
	$topic1->topic_desc = 'Географія  — комп­лекс дійсних и об­ще­ст­вен­ных на­ук, изу­чаю­щих струк­ту­ру, функ­цио­ни­ро­ва­ние и эво­лю­циюгео­гра­фи­че­ской обо­лоч­ки, взаи­мо­дей­ст­вие и рас­пре­де­ле­ние в про­стран­ст­ве при­род­ных и при­род­но-об­ще­ст­вен­ных гео­сис­тем и ихком­по­нен­тов. География изучает поверхность Земли, её природные условия, распределение на ней природных объектов , населения, экономических ресурсов';
	R::store($topic1);

	$topic2 = R::dispense('topics');
	$topic2->name = 'Історія';
	$topic2->topic_desc = 'История — область знаний, а также гуманитарная наука, занимающаяся изучением человека (его деятельности, состояния,мировоззрения, социальных связей, организаций и так далее) в прошлом. В более узком смысле  история — наука, изучающая всевозможные источники о прошломдля того, чтобы установить последовательность событий, объективность описанных фактов и сделать выводы о причинах событий.';
	R::store($topic2);

	$topic3 = R::dispense('topics');
	$topic3->name = 'Хімія';
	$topic3->topic_desc = 'Химия — одна из важнейших и обширных областей естествознания, наука о веществах, их составе и строении, их свойствах, зависящихот состава и строения, их превращениях, ведущих к изменению состава — химических реакциях, а также о законах и закономерностях, которым эти превращенияподчиняются.';
	R::store($topic3);

	$topic4 = R::dispense('topics');
	$topic4->name = 'Математика';
	$topic4->topic_desc = 'Математика - наука об отношениях между объектами, о которых ничего не известно, кроме описывающих их некоторых свойств, — именнотех, которые в качестве аксиом положены в основание той или иной математической теории. Исторически сложилась на основе операций подсчёта, измерения иописания формы объектов.';
	R::store($topic4);
} # end of if(topics)
...
if(R::count('tests') == 0)
{
	...
	$test1->test_desc = 'Цей тест зроблено для того, щоб учні могли перевірити свої знання столиць Австралії і Океанії для підготовки до наступноїсамостійної роботи))';
	...
	$test3->test_desc = 'Наша історія - те, що повинен знати кожен громадянин нашої країни. Адже, як говориться, без нашого минулого у нас не будемайбутнього. Давайте ж дамо можливість цьому майбутньому з\'явитися!!!';
